    ...
    <>
      ...
      >
        <div className="flex flex-row items-center justify-between gap-4">
          <p className="text-[20px] font-[600] leading-[30px]">
            Trading Market
          </p>
        ...
        {DATA.map((item, i) => (
          <ROW {...item} key={i} onClick={() => setGraph(true)} />
        ))}
      </div>
      <Modal open={graph} handleClose={handleCloseGraph}>
        <Graph />
      </Modal>
    </>
  );
...
const ROW = ({
  ...
  <div
    className="grid grid-cols-5 gap-1 sm:gap-2 md:gap-4 items-center cursor-pointer"
    onClick={onClick}
  >
